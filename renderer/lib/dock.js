...
export function makeDock(ws, {
  ...
  const wsTop = ws.querySelector('.ws-top');
  const wsBottom = ws.querySelector('.ws-bottom');
  const tl = ws.querySelector('.z-tl');
  const tr = ws.querySelector('.z-tr');
  const bl = ws.querySelector('.z-bl');
  const groups = ws.querySelector('.z-groups');

  let _topCol = topCol;
  let _topColFrac = null;       // fraction of top-row width; overrides px when set
  let _topFrac = topFraction;   // fraction of available height; null once dragged
  ...
  function bindDrag(name, update) {
    const handle = ws.querySelector(`[data-split="${name}"]`);
    handle.addEventListener('mousedown', (e) => {
      e.preventDefault();
      handle.classList.add('dragging');
      const move = (ev) => { update(ev); apply(); };
      const up = () => {
  ...
  apply();
  return {
    zones: { tl, tr, bl, br: ws.querySelector('.z-br'), groups },
