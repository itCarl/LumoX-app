...
export class QlcPlusImporter extends FixtureImporter {
  ...
  parse(input) {
    ...
    for (const block of findBlocks(xml, 'Channel', { topLevel: true })) {
...
function mapQlcGroup(group, byte, name) {
...
function buildCapability(group, name, min, max, label, openTag) {
 ...
 */
function findBlocks(xml, tag, { topLevel = false } = {}) {
  const out = [];
