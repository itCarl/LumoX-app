...
import type { MidiControllerOptions } from './MidiController';
import type { Group } from '../../show/Group';

const log = createLogger('ApcMiniMk2');

/** A user-bound pad action + LED color. */
...
export interface BindPadOptions {
  press?: () => void;
  release?: () => void;
  color?: number;
}

/** Inbound note message (only the fields we use). */
interface NoteMessage {
  note: number;
  velocity?: number;
}

/** Inbound control-change message (only the fields we use). */
interface CCMessage {
  controller: number;
  value: number;
}
...
export class ApcMiniMk2 extends MidiController {
  ...
  shift: boolean;
  _padBindings: Map<number, PadBinding>;
  _lastFaderValue: Map<number, number>;

  constructor(opts: MidiControllerOptions) {
    super(opts);
    this.shift = false;
  ...
  bindPad(note: number, { press, release, color = ApcMiniMk2.COLOR.white }: BindPadOptions = {}): void {
  ...
  bindScenesRow(row: number, sceneIds: string[], color = ApcMiniMk2.COLOR.cyan): void {
  ...
  // ---- handler binding -----------------------------------------------
  _bindHandlers(): void {
    this._on('noteon',  (m: NoteMessage) => this._onNoteOn(m));
    this._on('noteoff', (m: NoteMessage) => this._onNoteOff(m));
    this._on('cc',      (m: CCMessage) => this._onCC(m));
  }

  _onNoteOn({ note, velocity }: NoteMessage): void {
    if (velocity === 0) return this._onNoteOff({ note });
    // Pads
    if (note >= 0 && note <= 63) {
      const b = this._padBindings.get(note);
      if (b) {
        try { b.press?.(); } catch (e) { log.error('pad press:', (e as Error).message); }
  ...
  _onNoteOff({ note }: NoteMessage): void {
    if (note >= 0 && note <= 63) {
      const b = this._padBindings.get(note);
      if (b) {
        try { b.release?.(); } catch (e) { log.error('pad release:', (e as Error).message); }
  ...
  _onCC({ controller, value }: CCMessage): void {
  ...
  _onTrackPress(i: number): void {
    // Flash-to-100% on a group while held — release restores
    const g = this._groupByIndex(i);
    if (!g || !this.patch) return;
    (g as any)._mk2Saved = g.fixtures(this.patch).map((fx) => fx.get('intensity'));
    g.setIntensity(this.patch, 255);
    g.apply(this.patch, this.engine.universes);
  }
  _onTrackRelease(i: number): void {
    const g = this._groupByIndex(i);
    if (!g || !this.patch || !(g as any)._mk2Saved) return;
    const saved = (g as any)._mk2Saved as number[];
    g.fixtures(this.patch).forEach((fx, k) => fx.set('intensity', saved[k] ?? 0));
    g.apply(this.patch, this.engine.universes);
    (g as any)._mk2Saved = null;
  }

  // ---- LED helpers ----------------------------------------------------
  _setPadColor(note: number, color: number): void {
    this.output?.noteOn(note, color & 0x7f, 0);
  }
  _setPadColorChannel(note: number, color: number, ch = 0): void {
    // MK2 uses MIDI channel to select animation: 0 static, 1..6 various blinks/pulses.
    this.output?.noteOn(note, color & 0x7f, ch & 0x0f);
  }
  _setTrackLed(i: number, color: number): void {
    this.output?.noteOn(100 + i, color & 0x7f, 0);
  }
  _setSceneLed(i: number, color: number): void {
    this.output?.noteOn(112 + i, color & 0x7f, 0);
  }

  _refreshSceneLed(i: number): void {
  ...
  // ---- helpers --------------------------------------------------------
  _allGroups(fn: (g: Group) => void): void {
  ...
  _groupByIndex(i: number): Group | null {
