...
import type { Output } from '../src/outputs/Output';
...
// ---- command dispatcher -------------------------------------------------
type CommandFn = (tokens: string[]) => void | Promise<void>;
...
function num(s: string | undefined, min?: number, max?: number): number {
  const n = Number(s);
...
const int = (s: string | undefined, lo?: number, hi?: number): number => Math.round(num(s, lo, hi));
const u8 = (s: string | undefined): number => int(s, 0, 255);
...
`);
  ...
  async midi([sub, ...rest]) {
    if (sub === 'list') {
      ...
      for (const c of midi.list() as any[]) {
  ...
  effect([sub, ...rest]) {
    if (sub === 'list') {
      for (const fx of engine.groupEffects!.list()) {
        console.log(`  ${fx.id.padEnd(14)} ${fx.name.padEnd(18)} group=${(fx.group as any)?.id}  opacity=${fx.opacity}  enabled=${fx.enabled}`);
      }
      return;
    }
    if (sub === 'stop') {
