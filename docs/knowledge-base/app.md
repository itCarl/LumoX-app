...
| `views/library.ts` | FIXTURE LIBRARY — vendor accordion, patch form; header **grid** button opens the matrix/strip generator; **Custom** (user) fixtures carry an inline delete (confirm dialog → `lumox:library:remove`) |
...
- **Panel window** (`renderer/panel.html` + `panel-window.ts`, main:
  ...
  `settings` (`views/settings-modal.ts` → `buildSettingsBody`), `group-order`
  (`views/group-order-modal.ts` → `buildGroupOrderBody`), and `create-matrix`
  (`views/create-matrix-modal.ts` → `buildCreateMatrixBody` — the matrix/strip
  generator, see [fixtures.md](fixtures.md)).
