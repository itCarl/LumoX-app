...
`renderer/lib/audio-tempo.ts`, `renderer/index.ts` (titlebar clock),
`renderer/views/settings-modal.ts` (Tempo section), `src/midi/EasyMidiBackend.ts`
...
- **Audio** — `renderer/lib/audio-tempo.ts` runs only in the renderer (Web Audio has
  no main equivalent). It samples low-band energy, flags onsets above a rolling
  average (with a refractory gap), folds inter-onset intervals into 70–180 BPM, and
  pushes the most common estimate via `lumox:transport:audioBpm`. Main applies it
  only while `audio` is the source. Mic denial reverts the source to `manual`.
...
- Audio detection is energy-based and approximate — good for locking to a clear beat,
  not a substitute for a real clock. It uses the default input device (no per-device
  picker yet).
