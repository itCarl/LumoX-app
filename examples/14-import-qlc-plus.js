...
import { ImporterRegistry, FixtureLibrary } from '../src/index.js';
...
console.log('Using importer:', importer.constructor.FORMAT);
...
mode.channels.forEach((c, i) => {
  const caps = c.capabilities.map((p) => `[${p.min}-${p.max}] ${p.label}`).join('  ');
